...
- BFS
- 한 사이클 씩 반복하여 체크하고 목표 지점에 도달했을 때 사이클 횟수를 반환한다.
- 무한 루프 방지를 위해 방문 여부를 체크한다.

## 해결방법

- 특정 지점까지 오기까지 위해 걸린 이동 횟수를 저장할 `records` 2차원 배열을 만든다.
- 큐의 역할을 할 배열을 생성한 후 큐에 아무런 요소도 남아있지 않을 때까지 반복문을 순회한다.
- 큐에서 요소 하나를 꺼내 특정 작업을 반복한다.
  - 큐에서 꺼낸 요소의 인접 요소들의 방문 여부를 체크한다.
  - 방문하지 않았던 요소라면 해당 요소를 방문 처리하고 큐에 해당 요소를 집어 넣는다.
  - 큐에 요소를 집어 넣음과 동시에 `records` 2차원 배열에 해당 요소까지 도달하는데 소요된 반복 횟수를 저장한다.
    - 여기서, 반복 횟수는 꺼낸 요소의 반복 횟수 + 1이다.
- 큐를 꺼내고 작업을 수행하고 큐에 요소를 집어 넣는 과정을 반복하다가 목표 지점에 도달했을 때 정답을 반환하는데 정답은 이전과 동일하게 꺼낸 요소의 반복 횟수 + 1이다.
- 큐가 비었음에도 값이 반환되지 않았다면 목표 지점에 도달할 수 없는 상황이므로 -1을 반환한다.
