...
<img src="1.png" alt="콜라 문제 예시" width="500" />
...
- 추가로 콜라를 가져올 수 있는 병의 개수일 때 빈 병, 교환 가능한 병, 새로운 콜라의 수를 각각 구한다.
- 교환 가능한 총 병의 수는 입력으로 주어지는 최소 교환 가능 병의 수로 현재 빈 병의 수를 나눈 몫에 최소 교환 가능 병의 수를 곱한 값이다.
- 새로운 콜라의 수는 최소 교환 가능 병의 수로 현재 빈 병의 수를 나눈 몫에 입력으로 주어지는 최소 교환 가능 병의 수 당 받을 수 있는 콜라의 수를 곱한 값이다.

## 해결방법

- 추가로 콜라를 받을 수 있는 상황 즉, 현재 빈 병의 수가 `a` 보다 크거나 작은 경우를 계속해서 순회하는 반복문을 작성한다.
- 아이디어에 적어놓은 것을 기반으로 빈 병, 교환 가능한 병, 새로운 콜라의 수를 각각 구한다.
- 새로운 콜라의 수를 정답에 누적시킨다.
