...
- 정현이가 원하는 품목을 `key` 그리고 해당 품목의 개수를 `value`로 하는 객체를 하나 만든다.
- 10일을 넘어가는 일치 상황에 대해서는 고려하지 않기 때문에 시작점과 끝점을 정의한다.
  - 시작점과 끝점을 옮기되 끝점이 할인 행사 마지막 날짜를 넘어가는 순간 반복문을 종료한다.
- 시작점과 끝점을 통한 반복문 순회를 통해 해당 날짜에 할인 품목을 `key` 그리고 품목의 개수를 `value`로 하는 객체를 만든다.
- `every` 메서드를 통하여 위 두 객체의 값이 일치하는지 확인하고 일치할 경우 정답에 +1 한다.
- 일치하는 경우가 없을 경우 0을 반환한다.
